...
Rsync是一种快速而非常通用的文件复制工具。它可以通过任何远程shell或远程rsync守护进程从本地复制到另一个主机。它通过仅发送源文件和目的地中的现有文件之间的差异来减少通过网络发送的数据量。Rsync广泛用于备份和镜像以及日常使用的改进的复制命令。
